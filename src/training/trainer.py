...
class Trainer:
    ...
    def train(self):
        ...
        corpus = (
            self.corpus_builder.load_corpus()
        )

        print(
            f"Caracteres cargados: "
            f"{len(corpus)}"
        )
        ...
        tokens = (
            self.tokenizer.manual_tokenizer(
                cleaned_text
            )
        )
        ...
        print(
            "eduGPTIA ya tiene vocabulario "
            "y embeddings."
        )

        print(
            "El entrenamiento real de los pesos "
            "comenzará en la siguiente etapa."
        )